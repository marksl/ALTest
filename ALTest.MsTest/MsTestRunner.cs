            ...
            using (var w = new XmlTextWriter(fileName, Encoding.UTF8))
            {
                ...
                w.WriteStartDocument();
                {
                    w.WriteStartElement("TestRun");
                    {
                        ...
                        var testIdToExecutionId = new Dictionary<string, Tuple<string, string, bool>>();
                        ...
                        foreach (var r in results)
                        {
                            w.WriteStartElement("UnitTest");
                            {
                                ...
                                testIdToExecutionId.Add(testId, new Tuple<string, string, bool>(executionId, r.TestName, r.TestPassed));

                                w.WriteAttributeString("name", r.TestName);
                                w.WriteAttributeString("storage", "ASSEMBLY-TODO");
                                ...
                                w.WriteAttributeString("codeBase", "ASSEMBLY-TODO");
                                w.WriteAttributeString("adapterTypeName", "Microsoft.VisualStudio.TestTools.TestTypes.Unit.UnitTestAdapter, Microsoft.VisualStudio.QualityTools.Tips.UnitTest.Adapter, Version=11.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a");
                        ...
                        string todo = DateTime.Now.ToString(CultureInfo.InvariantCulture);
                        foreach (string testId in testIdToExecutionId.Keys)
                        {
                            var tuple = testIdToExecutionId[testId];

                            string executionId = tuple.Item1;
                            string testName = tuple.Item2;
                            bool passed = tuple.Item3;
                            ...
                            w.WriteAttributeString("duration", "00:00:00"); // TODO
                            w.WriteAttributeString("startTime", todo);
                            w.WriteAttributeString("endTime", todo);
                            w.WriteAttributeString("testType", "13cdc9d9-ddb5-4fa4-a97d-d965ccfc6d4b");
                            w.WriteAttributeString("outcome", passed ? "Passed" : "Failed");
                            w.WriteAttributeString("testListId", testListId);
